...
import { ApiResponse, BypassRequest, BypassRequestInput, HandleBypassInput } from "@/types/bypass";
...
export const useBypassRequest = () => {
  ...
  const getBypassRequestById = async (id: number) => {
    try {
      setLoading(true);
      const response = await api.get<ApiResponse<BypassRequest>>(`/bypass/${id}`);
  ...
  const requestBypass = async (data: BypassRequestInput) => {
    try {
      setLoading(true);
      const response = await api.post<ApiResponse<BypassRequest>>("/bypass/request", data);
      return response.data;
    } catch (err) {
  ...
  const handleBypassRequest = async (data: HandleBypassInput) => {
    try {
      setLoading(true);
      const response = await api.put<ApiResponse<BypassRequest>>("/bypass/handle", data);
      return response.data;
    } catch (err) {
