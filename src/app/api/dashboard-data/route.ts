...
async function getOutlets() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `Failed to fetch outlets: ${res.status} ${res.statusText}`,
        errorData
      );
      return { data: [] };
    }
...
async function getNewCustomersThisMonth() {
  ...
  try {
    ...
    const newCustomers = data.users.filter(
      (user: any) => new Date(user.createdAt) >= firstDayOfMonth
    );
...
async function getAllOrders() {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/orders/show-order`,
      {
        cache: "no-store",
        headers: {
          "Content-Type": "application/json",
...
function getItemsCreatedThisMonth(items: any[]) {
  ...
  return items.filter((item) => new Date(item.createdAt) >= firstDayOfMonth)
    .length;
}
...
function extractOrdersArray(ordersResponse: any): any[] {
  if (ordersResponse?.data?.data?.data) {
    ...
    return Array.isArray(ordersResponse.data.data)
